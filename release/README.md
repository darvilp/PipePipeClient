...
The current counter is base version code `1110`, producing arm64 code `111004`, with version name `5.3.1-beta-unofficial.3`. Increment the fork counter for every subsequent distributed build, independently of the upstream version name. Never overwrite an old APK or reuse a public release tag.
...
2. Tag the tested client commit, for example `all-features/5.3.1-beta-unofficial.3`, refusing an existing tag.
...
Keep previous releases and local APK copies. A safe rollback may require rebuilding corrected source with a higher version code; do not assume an older APK can downgrade newer database state. Other ABI splits require their own artifact checks and runtime acceptance before distribution.
